...
# Short on purpose: the first test replays this journal twenty times, and every
# extra record is twenty more round trips across the pipe.
TRADE_COUNT = 15
...
# rather than inlined, because it is the exact value this milestone exists to
...
def sealed_data(sbx_cli, sbx_home: Path, tmp_path: Path) -> str:
    """Seal the journal every test here replays, and return its digest."""
    source = journals.write_journal(tmp_path / "market.jsonl", trades())
